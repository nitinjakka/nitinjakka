###python /data/dump/purge_atlas_deleted_entities_with_logging_v4.py 100000 100 CLUSTER1 `date -d "6 months ago" "+%Y-%m-%d"` 2> /dev/null &
...
def get_deleted_entities(offset, atlas_host_name, nbkid, password, total_entities_to_delete, atlas_hosts):
    ...
    count_output = subprocess.check_output(count_url, shell=True)
    ...
    curl_command = """curl -i -L --negotiate -u:{} -k -X POST {} -H 'Content-Type: application/json' -d '{}' """.format(nbkid, url, data_string)

    try:
        deleted_entities = subprocess.check_output(curl_command, shell=True)
#        logging.info(deleted_entities)
#        with open('/data/logs/atlas-purge/purge_curl_entries.txt', 'a') as file:
#            file.write(deleted_entities)
...
#    def run_cmd(batch):
#        cmd = "curl -L --negotiate -u:{0} -k -X PUT {2} -H 'Content-Type: application/json' -d '{3}'".format(nbkid, password, url_purge, json.dumps(batch))
#        logging.info(cmd)
#        with open('/data/logs/atlas-purge/purge_curl_entries.txt', 'a') as file:
#            file.write(cmd)
#        with open('/data/logs/atlas-purge/purge_curl_entries.txt', 'a') as output_of_curl_command:
#            subprocess.call(cmd, shell=True, stdout=output_of_curl_command, stderr=output_of_curl_command)
...
#        run_cmd(batch)
    ...
    def run_cmd(batch, process_id):
        log_filename = '/data/logs/atlas-purge/purge_curl_entries-{}.txt'.format(process_id)
        cmd = "curl -L --negotiate -u:{} -k -X PUT {} -H 'Content-Type: application/json' -d '{}'".format(nbkid, url_purge, json.dumps(batch))
        with open(log_filename, 'a') as output_of_curl_command:
            output_of_curl_command.write(cmd + '\n')
            subprocess.call(cmd, shell=True, stdout=output_of_curl_command, stderr=output_of_curl_command)

    # Splitting batches into 10 parts for parallel processing
    num_processes = 10
    split_batches = [batches[i::num_processes] for i in range(num_processes)]

    # Creating and starting processes
    processes = []
    for i in range(num_processes):
        p = multiprocessing.Process(target=run_cmd, args=(split_batches[i], i))
        processes.append(p)
        p.start()
...
def get_deleted_typeName_count(atlas_host_name, nbkid, password, typeName):
    ...
    command = [
        'curl', '-i', '-L', '--negotiate', '-u:',nbkid, '-k', '-X', 'PUT', '-H', 'Content-Type: application/json', '-d', json.dumps(data), url
    ]
